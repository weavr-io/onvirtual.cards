export default {
  ...
  profileId: {
    ...
    managed_accounts_consumers:
      process.env.MANAGED_ACCOUNTS_CONSUMERS_PROFILE_ID,
    managed_accounts_corporates:
      process.env.MANAGED_ACCOUNTS_CORPORATES_PROFILE_ID,
    transfers: process.env.TRANSFERS_PROFILE_ID,
    send: process.env.SEND_PROFILE_ID
  },
  app: {
    kyb_required: process.env.KYB_REQUIRED
      ? JSON.parse(process.env.KYB_REQUIRED)
      : true,
    view_register: process.env.VIEW_REGISTER
      ? JSON.parse(process.env.VIEW_REGISTER)
      : true,
